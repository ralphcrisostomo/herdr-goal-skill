...
**Distrust the shell before you distrust the lead.** In one run the token-proxy hook wrapping bash (`rtk`) returned *fabricated* file contents — a `cat -n` came back with lines rewritten and an invented `// MUTATION: ...` comment that did not exist on disk — and separately made `grep` silently drop matching lines, which made real mutation runs look like they produced no output. `git status` was clean throughout, so nothing flagged it. Read files with the Read tool, or `rtk proxy <cmd>` to bypass the filter, whenever a claim is load-bearing; never audit a lead's work through plain wrapped `cat`/`grep`/`git show`. If you catch this, tell the lead too — its own review rounds were reading through the same proxy — and tell the user, because every other session in that repo is affected.
