...
**Distrust a lead's unfalsifiable environmental diagnosis.** When a lead attributes a failure to something that conveniently cannot be tested from its sandbox ("the platform suppresses automated sessions"), do not let that close the investigation while the symptom is user-visible: get one out-of-band observation first (the owner's real browser, a second instrument). In one run the "bot detection" story was half-right yet masked a real code bug (an Array pushed where gtag.js requires an `arguments` object) that reproduced identically in the owner's browser; the owner's screenshot refuted the diagnosis in one glance after multiple polling rounds had gone nowhere.
